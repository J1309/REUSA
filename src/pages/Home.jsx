...
function Hero() {
  ...
  return (
    <section className="relative min-h-[92svh] overflow-hidden bg-ink pt-28 pb-20 sm:pt-36 sm:pb-24 lg:pt-40 lg:pb-28">
      ...
      <div ref={copy} className="relative mx-auto max-w-7xl px-6 lg:px-10 h-full flex flex-col justify-between">
        <div className="grid gap-12 lg:grid-cols-[1.3fr_1fr] items-center">
          ...
          <div className="max-w-2xl text-sand">
            <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-sand/15 bg-sand/10 px-3.5 py-1.5 backdrop-blur-md">
              <span className="size-1.5 rounded-full bg-accent animate-pulse" />
              <span className="text-[11px] font-medium tracking-[0.2em] uppercase text-sand/80">
                Realtor LG Private Office · Est. 2008
              </span>
            </div>

            <h1 className="font-display text-[clamp(2.5rem,6vw,4.8rem)] leading-[1.02] tracking-tight text-sand">
              <SplitText text="Architectural homes," />
              <br />
              <SplitText text="curated with intention." delay={0.2} className="italic text-accent" />
            </h1>

            <Reveal delay={0.4} className="mt-7 text-base sm:text-lg leading-relaxed text-sand/75 max-w-xl">
              <p>
            ...
            <Reveal delay={0.6} stagger={0.1} className="mt-9 flex flex-wrap items-center gap-3.5">
              <Link
                to="/listings"
                className={`${btnPrimary} bg-sand text-ink hover:bg-accent shadow-lg hover:shadow-accent/20`}
              >
                Explore Portfolio
              </Link>
              <a href="#contact" className={`${btnGhost} text-sand backdrop-blur-sm border-sand/25 hover:bg-sand/10`}>
                Schedule Consultation
              </a>
            </Reveal>

            {/* Authority Benchmark Bar */}
            <Reveal delay={0.8} className="mt-12 pt-8 border-t border-sand/15 grid grid-cols-3 gap-6 text-sand/80">
              <div>
                <p className="font-display text-2xl sm:text-3xl text-sand">$520M+</p>
                <p className="text-[11px] text-sand/50 uppercase tracking-wider mt-0.5">Transactions Closed</p>
              </div>
              <div>
                <p className="font-display text-2xl sm:text-3xl text-accent">98%</p>
                <p className="text-[11px] text-sand/50 uppercase tracking-wider mt-0.5">List-to-Sale Ratio</p>
              </div>
              <div>
                <p className="font-display text-2xl sm:text-3xl text-sand">6 States</p>
                <p className="text-[11px] text-sand/50 uppercase tracking-wider mt-0.5">Licensed Principals</p>
              </div>
            </Reveal>
          </div>
...
function SearchBar() {
  ...
  return (
    <div className="relative z-20 mx-auto -mt-10 sm:-mt-14 max-w-6xl px-6 lg:px-10">
      <Reveal>
        <form
          onSubmit={submit}
          className="grid gap-3 rounded-3xl border border-stone/80 bg-white/95 p-3.5 sm:p-4 shadow-[0_30px_70px_-25px_rgba(12,31,28,0.25)] backdrop-blur-xl sm:grid-cols-2 lg:grid-cols-[1.5fr_1fr_1fr_auto]"
        >
          {/* Location input */}
          ...
          {/* Submit Button */}
          <button className={`${btnPrimary} h-full min-h-12 rounded-2xl shadow-md hover:shadow-lg flex items-center justify-center gap-2`}>
...
function Featured() {
  ...
  return (
    <section className="mx-auto max-w-7xl px-6 py-16 sm:py-24 lg:py-28 lg:px-10">
      <div className="mb-10 sm:mb-14 flex flex-wrap items-end justify-between gap-6">
        <Reveal>
          ...
          <h2 className="mt-2 font-display text-[clamp(2rem,4vw,3.2rem)] leading-tight text-ink">
      ...
      <Reveal stagger={0.15} y={50} className="grid gap-7 md:grid-cols-2 lg:grid-cols-3">
        {featured.map((p) => (
    ...
    <section className="bg-ink text-sand py-20 sm:py-28">
      <div className="mx-auto max-w-7xl px-6 lg:px-10">
        <Reveal className="max-w-2xl">
          ...
          <h2 className="mt-2 font-display text-[clamp(2rem,4vw,3.2rem)] leading-tight">
            Eighteen years, one market at a time.
          </h2>
          <p className="mt-6 text-lg leading-relaxed text-sand/65">
        ...
        <Reveal stagger={0.12} className="mt-14 sm:mt-20 grid grid-cols-2 gap-10 lg:grid-cols-4">
          {stats.map((s) => (
            <div key={s.label} className="border-t border-sand/15 pt-6">
              <Counter to={s.value} suffix={s.suffix} className="font-display text-[clamp(2.4rem,5vw,3.6rem)] text-sand" />
        ...
        {lifestyle.map((l) => (
          <figure key={l.src} className="group overflow-hidden rounded-3xl border border-stone/80 bg-white p-3.5 shadow-sm transition-all hover:shadow-md">
            ...
            <figcaption className="p-4 pt-5">
              ...
              <p className="mt-1 text-sm text-muted leading-relaxed">{l.copy}</p>
            </figcaption>
          </figure>
        ))}
      </Reveal>
...
function Testimonials() {
  ...
  return (
    <section className="bg-deep text-sand py-20 sm:py-28">
      <div className="mx-auto max-w-4xl px-6 text-center lg:px-10">
        ...
        <p className="mt-8 text-sm text-sand/60">
...
function CTA() {
  return (
    <section className="mx-auto max-w-7xl px-6 py-16 sm:py-24 lg:py-28 lg:px-10">
      <Reveal className="overflow-hidden rounded-[2.5rem] bg-white p-10 text-center shadow-[0_30px_70px_-30px_rgba(12,31,28,0.25)] border border-stone/80 md:p-20">
        <span className="text-xs uppercase tracking-[0.25em] text-sea font-semibold">Private Advisory</span>
        <h2 className="mx-auto mt-2 max-w-2xl font-display text-[clamp(2rem,4.5vw,3.4rem)] leading-tight text-ink">
